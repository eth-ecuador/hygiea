...
async function main() {
  ...
  if (fs.existsSync(artifactPath)) {
    ...
    const frontDoctorDir = path.join(__dirname, "..", "front-doctor", "contracts");
    if (!fs.existsSync(frontDoctorDir)) {
      fs.mkdirSync(frontDoctorDir, { recursive: true });
    }

    const abiFileFrontDoctor = path.join(frontDoctorDir, "MedicalRecords.json");
    fs.writeFileSync(
      abiFileFrontDoctor,
      JSON.stringify({ abi: artifact.abi, address: address }, null, 2)
    );

    console.log("ABI copiado para front-doctor:", abiFileFrontDoctor);
    console.log();
  }
  ...
  console.log("=".repeat(60));
  console.log();
  console.log("1. Actualiza front-doctor/.env.local con la dirección del contrato:");
  console.log(`   NEXT_PUBLIC_CONTRACT_ADDRESS=${address}`);
  console.log();
  console.log("2. Actualiza front-doctor/config/wagmi.ts con la dirección del contrato");
  console.log(`   export const contractAddress = '${address}'`);
  console.log();
  console.log("3. Inicia el frontend:");
  console.log("   cd front-doctor && npm run dev");
  console.log();
  console.log("4. Conecta tu wallet a Sapphire", hre.network.name);
  console.log();
